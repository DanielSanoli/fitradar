...
export function StructuredNutritionPanel({ programId, canWrite = true }: StructuredNutritionPanelProps) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setPlan(data);
      setActiveMealId((current) => current ?? data.meals[0]?.id ?? null);
      setState("content");
    } catch (e) {
      ...
      <PanelState state={state} message={error} onRetry={load}>
        {plan ? (
          <div className="space-y-5">
            ...
            {plan.meals.map((meal) => (
              <div key={meal.id} className="rounded-[14px] border border-border bg-card p-4">
                ...
                <div className="mt-3">
                  <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Totais da refeição</p>
                  <NutrientTotalsDisplay totals={meal.totals} compact className="mt-2" />
                </div>
              </div>
            ))}

            {canWrite && activeMealId ? (
              <div className="rounded-[14px] border border-primary/30 bg-primary/5 p-4">
                <p className="mb-3 text-sm font-semibold">Adicionar alimento à refeição selecionada</p>
