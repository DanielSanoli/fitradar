      ...
      <PanelState state={state} message={error} onRetry={load}>
        {plan ? (
          <div className="space-y-4">
            ...
            {plan.meals.length === 0 ? (
              <p className="text-sm text-muted-foreground">Seu coach ainda não montou refeições neste plano.</p>
            ) : (
              plan.meals.map((meal) => (
                <article
                  key={meal.id}
                  className="rounded-[14px] border border-border bg-card p-4 shadow-[0_4px_20px_rgba(0,0,0,0.2)]"
                >
